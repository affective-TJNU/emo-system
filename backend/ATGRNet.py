...
import torch
import math
from base import TemporalConvNet, SELayer, ReadoutLayer
import copy
from base import laplacian_torch, custom_init, truncated_normal_, calculate_adj_matrix
...
class ATGRNet(nn.Module):
    def __init__(self, args, device='cpu'):
        ...
        # feature_len=x.shape[2]//self.windows_num
        # total_usable_features = feature_len * self.windows_num
        # 裁剪 x 到可用的最大特征长度
        # x = x[:, :, :total_usable_features]

        self.args = args
        self.device = args.device
        self.windows_num = args.windows_num

        tem_feature_len = args.feature_len // self.windows_num
        total_usable_features = tem_feature_len * self.windows_num

        feature_len=total_usable_features
        ...
        self._laplacian_tick = 0
        self._laplacian_refresh_every = 4

        self.BN = nn.BatchNorm2d(feature_len)

        for i in range(self.windows_num):
            self.SSTG_array.append(SSTGBlockResidual(
                args, math.ceil(feature_len/self.windows_num), device))
        ...
        self.tcn = TemporalConvNet(
            (args.graph_out*2)*args.channels_num//args.rsr+math.ceil(feature_len/self.windows_num)*args.channels_num, tcn_hidden_channels)
        #2014/9/3 滑动窗口为2 改形状为（8.360）
        #fc_in_dim = (self.windows_num)*tcn_hidden_channels[-1]  # *2
        fc_in_dim=360#360 #deap修改为120
        hidden_dim = int(getattr(args, 'atgrnet_hidden', 1024))
        ...
        self.classify_layer2 = nn.Linear(hidden_dim, args.nclass)

    def _refresh_laplacian(self):
        self.laplacian = laplacian_torch(
            self.A.detach()).to_sparse().to(self.device)
    ...
    def forward(self, x):
        ...
        self._laplacian_tick += 1
        if self._laplacian_tick % self._laplacian_refresh_every == 0:
            self._refresh_laplacian()
...
class SSTGBlockResidual(nn.Module):
    def __init__(self, args, in_channels, device='cpu'):
        ...
        tem_feature_len = args.feature_len // windows_num
        total_usable_features = tem_feature_len * windows_num
        ...
        # graph convolution
        in_dim = math.ceil(feature_len/args.windows_num)
        self.graph_layer_g = PolyGCLayer(
            in_channels, args.graph_out, 25, args.pooling_size, feature_len, device)
...
class PolyGCLayer(nn.Module):
    def __init__(self, in_channels, out_channels, poly_degree, pooling_size, feature_len, device):
        ...
        """
        super().__init__()
        self.weight = nn.Parameter(torch.empty(poly_degree * in_channels, out_channels))
        self.bias = nn.Parameter(torch.empty(1, 1, out_channels))
        self.pooling_size = pooling_size
        self.poly_degree = poly_degree
        self.W_t = nn.Parameter(torch.empty(in_channels, in_channels))
...
class GraphConvolution(nn.Module):
    ...
    def __init__(self, num_in, num_out, bias=False, device='cpu'):
