...
class Band_Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        out = torch.tensor(
            np.matmul(x.detach().cpu().numpy(), out.detach().cpu().numpy()),
            device=x.device,
            dtype=x.dtype,
        )
...
class Feature_Extractor(nn.Module):
    ...
    def __init__(self, args, device):
        ...
        self.args = args
        self.device = device
        if args.dataset in ('SEED', 'SEED_4CH'):
...
class Label_Classifier(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.args = args
        if args.dataset == 'SEED_4CH':
            self.pooling = nn.AdaptiveAvgPool2d((4, 10))
            self.fc_1 = nn.Linear(args.conv3_out_dim * 4 * 10, args.fc1_dim)
