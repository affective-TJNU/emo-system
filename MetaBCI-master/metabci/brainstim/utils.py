...
def _resolve_lsl_channel_format(name):
    """Map legacy cf_* format names to pylsl>=1.16 string keys."""
    from pylsl import info as pylsl_info

    if name in pylsl_info.string2fmt:
        return name
    legacy = {
        "cf_int16": "int16",
        "cf_string": "string",
    ...
    mapped = legacy.get(name)
    if mapped and mapped in pylsl_info.string2fmt:
        pylsl_info.string2fmt.setdefault(name, pylsl_info.string2fmt[mapped])
        return name
...
class LsLPort:
    ...
    def __init__(self) -> None:
        channel_format = _resolve_lsl_channel_format('cf_int16')
